class MovieRentingSystem {
    ...
    private final TreeSet<Node> resntset = new TreeSet<>(CMP);
    ...
    public void drop(int shop, int movie) {
        long k = key(shop, movie);
        Node nd = byPair.get(k);
        if (nd == null) return; // defensive
        resntset.remove(nd);
        availableByMovie
    ...
    public List<List<Integer>> report() {
        List<List<Integer>> reportAns = new ArrayList<>(5);
        Iterator<Node> it = resntset.iterator();
        for (int i = 0; i < 5 && it.hasNext(); i++) {
            Node n = it.next();
            reportAns.add(Arrays.asList(n.shop, n.movie));
        }
        return reportAns;
    }
}
